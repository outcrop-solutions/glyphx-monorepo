...
export function TableListItem({ projectDetails }) {
  ...
  return (
    <div
      onClick={handleClick}
      className="flex flex-row justify-between items-center bg-secondary-space-blue rounded py-2 px-6 border border-transparent hover:border-white hover:bg-secondary-midnight hover:cursor-pointer font-roboto font-normal text-[14px] leading-[16px] tracking-[0.01em] text-light-gray hover:text-white"
    >
      <p title="Project Name" className="w-40">
        {projectDetails.name}
      </p>
      {/* @ts-ignore */}
