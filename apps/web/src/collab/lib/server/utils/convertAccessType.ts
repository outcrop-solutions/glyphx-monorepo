...
export function roomAccessesToDocumentAccess(
  ...
  if (roomAccesses.includes(RoomAccess.RoomRead) && roomAccesses.includes(RoomAccess.RoomPresenceWrite)) {
 ...
 */
export function documentAccessToRoomAccesses(documentAccess: DocumentAccess): RoomAccess[] {
  if (documentAccess === DocumentAccess.FULL || documentAccess === DocumentAccess.EDIT) {
