...
export function usePaginatedDocumentsSWR(
  ...
  const getKey: SWRInfiniteKeyLoader = (pageIndex, previousPageData) => {
    ...
    if (pageIndex === 0) {
      return [clientDoc.getDocuments, documentsOptions];
    }

    // Current page is a later page, get next documents from `nextPage`
    return [clientDoc.getNextDocuments, {nextPage: previousPageData.nextPage}];
  };

  const fetcher = async <T extends (...args: any) => any>(func: T, ...args: Parameters<T>[]) => {
  ...
  const result = useSWRInfinite<GetDocumentsResponse>(getKey, fetcher, swrOptions);
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.documents.length < (documentsOptions?.limit || 10)) || atEnd;
