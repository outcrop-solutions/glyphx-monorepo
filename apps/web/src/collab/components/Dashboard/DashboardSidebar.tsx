...
import {DASHBOARD_DRAFTS_URL, DASHBOARD_GROUP_URL, DASHBOARD_URL} from '../../constants';
...
function SidebarLink({href, children, className, ...props}: SidebarLinkProps) {
  ...
  const isActive = useMemo(() => normalizeTrailingSlash(pathname) === normalizeTrailingSlash(href), [pathname, href]);
        ...
        <div className={styles.category}>
          ...
          <ul className={styles.list}>
            {groups.map((group) => {
              return (
                <li key={group.id}>
                  <SidebarLink href={DASHBOARD_GROUP_URL(group.id)} icon={<FolderIcon />}>
                    {group.name}
                  </SidebarLink>
                </li>
              );
