...
export function Whiteboard() {
  ...
  return (
    <ClientSideSuspense fallback={<DocumentSpinner />}>
      {() => <Canvas currentUser={session?.user ?? null} />}
    </ClientSideSuspense>
  );
}
...
function Canvas({currentUser, className, style, ...props}: Props) {
  ...
  const noteIds = useStorage<string[]>((root) => Array.from(root.notes.keys()), shallow);
      ...
      {!isReadOnly && (
        <div className={styles.toolbar}>
          ...
          <Tooltip content='Undo' sideOffset={16}>
            <Button disabled={!canUndo} icon={<UndoIcon />} onClick={history.undo} variant='subtle' />
          </Tooltip>
          <Tooltip content='Redo' sideOffset={16}>
            <Button disabled={!canRedo} icon={<RedoIcon />} onClick={history.redo} variant='subtle' />
          </Tooltip>
        </div>
      )}
    </div>
