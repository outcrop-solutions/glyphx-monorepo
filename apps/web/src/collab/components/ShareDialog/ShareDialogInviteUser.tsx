...
export function ShareDialogInviteUser({documentId, fullAccess, onSetUsers, className, ...props}: Props) {
  ...
  async function handleAddDocumentUser(id: DocumentUser['id']) {
    ...
    const {error} = await clientDoc.updateUserAccess({
      userId: id,
  ...
  return (
    <div className={clsx(className, styles.section)} {...props}>
      {fullAccess ? (
        ...
        <div className={styles.error}>You need full access to invite others.</div>
      )}
    </div>
  );
}
