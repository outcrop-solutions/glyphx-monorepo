...
export function ShareDialogDefault({
  ...
  async function handlePublicRead(newPublicRead: boolean) {
    ...
    const accessValue = newPublicRead ? DocumentAccess.READONLY : DocumentAccess.NONE;

    const {data, error} = await clientDoc.updateDefaultAccess({
  ...
  async function handlePublicEdit(newPublicEdit: boolean) {
    ...
    const accessValue = newPublicEdit ? DocumentAccess.EDIT : DocumentAccess.READONLY;
      ...
      {defaultAccess !== DocumentAccess.NONE ? (
        <>
          ...
          <div className={clsx(styles.section, styles.shareLinkSection)}>
            <Input className={styles.shareLinkInput} readOnly ref={shareInputRef} value={window.location.href} />
            <Button icon={copiedToClipboard ? <CheckIcon /> : <CopyIcon />} onClick={handleCopyToClipboard}>
              Copy
            </Button>
          </div>
        </>
