...
export function ToolbarInlineAdvanced({editor}: Props) {
  ...
  return (
    <>
      ...
      <Popover
        content={
          <LinkPopover onSubmit={toggleLink} onRemoveLink={toggleLink} showRemove={editor.getAttributes('link').href} />
        }
      >
        <Button
          variant='subtle'
  ...
  return (
    ...
    >
      ...
      <div className={styles.toolbarPopoverBar}>
        <Input className={styles.toolbarPopoverInput} value={value} onChange={(e) => setValue(e.target.value)} />
        {showRemove ? (
          <Button
            variant='secondary'
            onClick={(e) => {
