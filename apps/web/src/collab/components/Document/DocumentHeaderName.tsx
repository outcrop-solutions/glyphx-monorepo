...
import {ChangeEvent, ComponentProps, KeyboardEvent, useCallback, useState} from 'react';
import {EditIcon} from '../../icons';
import {useSelf} from 'liveblocks.config';
import {Tooltip} from '../../primitives/Tooltip';
import styles from './DocumentHeaderName.module.css';
import {useInitialDocument} from 'collab/lib/client';

interface Props extends ComponentProps<'div'> {
  onDocumentRename: (name: string) => void;
}

export function DocumentHeaderName({onDocumentRename, className, ...props}: Props) {
      ...
      {isReadOnly ? (
        ...
        <>
          ...
          <Tooltip content='Rename' sideOffset={30}>
            <button className={styles.renameButton} onClick={handleRenamingStart}>
              <EditIcon />
            </button>
          </Tooltip>
        </>
