...
export const RangeFilter = ({ setVisible, lastDroppedItem }) => {
  ...
  /**
   * SETS MAX MIN FILTER FOR SPECIFIC AXIS
   */
  function setMaxMinFilter() {
    // TODO: NEED ERROR CHECKING HERE FOR NON NUMERIC VALUES
    setFiltersApplied((prev) => {
      return [
        ...prev,
        {
          name: lastDroppedItem.key,
          min: min,
          max: max,
        },
      ];
  ...
  /**
   * DELETE CURRENT MAX MIN FILTER IN APPLIED FILTERS ARRAY
   * USING CURRENT FILTER VARIABLE
   */
  function deleteMaxMinFilter() {
    setFiltersApplied((prev) => {
      return prev.filter((internalFilter) => {
        // remove this range filter here
        return JSON.stringify(internalFilter) !== JSON.stringify(currentFilter);
      });
    });
    setVisible(false);
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex grow items-center justify-between">
        ...
        />
        <p className="text-light-gray font-roboto text-[10px] font-normal leading-[12px] text-center">
          -
        </p>
        <input
          onChange={(e) => {
      ...
      {/* DELETE */}
      {/* <svg
        onClick={deleteMaxMinFilter}
        className="rounded border border-transparent hover:border-white hover:cursor-pointer bg-secondary-space-blue"
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="none"
