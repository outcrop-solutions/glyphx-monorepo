...
// @ts-ignore
import {ArrowDownIcon, ArrowLeftIcon, ArrowRightIcon, ArrowUpIcon, Move3D} from 'lucide-react';
// import {screenshotModel} from '../Model/utils';

export const ModelControls = () => {
  const modelRunnerState = useRecoilValue(modelRunnerAtom);
  const {x, y, z} = useRecoilValue(selectedGlyphsAtom);
  ...
  return (
    ...
    <>
      ...
      <div className="absolute right-2 top-11 flex-col items-center space-y-2 z-[9999] pt-2">
        <div
          onClick={() => modelRunnerState.modelRunner.raise_model(10)}
          className="hover:bg-gray bg-secondary-blue rounded-full h-8 w-8 flex items-center justify-center cursor-pointer"
        >
          <ArrowUpIcon className="h-5 w-5" />
        </div>
        <div
          onClick={() => modelRunnerState.modelRunner.shift_model(-10)}
          className="hover:bg-gray bg-secondary-blue rounded-full h-8 w-8 flex items-center justify-center cursor-pointer"
        >
      ...
      <div className="absolute left-1/2 bottom-11 transform -translate-x-1/2 flex items-center space-x-2 z-[9999]">
        <div className="flex items-center justify-center cursor-pointer text-xs text-light-gray">X: {x}</div>
        <div className="flex items-center justify-center cursor-pointer text-xs text-light-gray">Y: {y}</div>
        <div className="flex items-center justify-center cursor-pointer text-xs text-light-gray">Z: {z}</div>
      </div>
    </>
    // )
  );
