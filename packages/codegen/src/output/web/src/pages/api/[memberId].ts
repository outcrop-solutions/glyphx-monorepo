...
import {getMember, createMember, updateMember, deleteMember } from 'lib/server/member';

const member = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (req.method) {
    ...
    case webTypes.constants.HTTP_METHOD.DELETE:
      return deleteMember(req, res, session);
    default:
      res.setHeader('Allow', [
