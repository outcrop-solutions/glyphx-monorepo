...
import {getAccount, createAccount, updateAccount, deleteAccount } from 'lib/server/account';
...
const account = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (req.method) {
    ...
    case webTypes.constants.HTTP_METHOD.PUT:
      return updateAccount(req, res, session);
    case webTypes.constants.HTTP_METHOD.DELETE:
      return deleteAccount(req, res, session);
    default:
      res.setHeader('Allow', [
