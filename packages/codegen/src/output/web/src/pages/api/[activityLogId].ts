...
import {getActivityLog, createActivityLog, updateActivityLog, deleteActivityLog } from 'lib/server/activityLog';

const activityLog = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (req.method) {
    ...
    case webTypes.constants.HTTP_METHOD.POST:
      return createActivityLog(req, res);
    case webTypes.constants.HTTP_METHOD.PUT:
      return updateActivityLog(req, res, session);
    case webTypes.constants.HTTP_METHOD.DELETE:
      return deleteActivityLog(req, res, session);
    default:
      res.setHeader('Allow', [
