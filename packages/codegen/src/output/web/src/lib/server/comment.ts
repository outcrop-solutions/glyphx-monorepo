...
export const getComments = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
...
export const updateComment = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { commentId} = req.query;
  ...
  try {
    const updatedComment = await commentService.updateComment(commentId as string, comment);
...
export const deleteComment = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { commentId} = req.query;
  if (Array.isArray(commentId)) {
    return res.status(400).end('Bad request. Parameter cannot be an array.');
  }
