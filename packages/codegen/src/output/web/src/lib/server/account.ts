...
export const getAccounts = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
...
export const updateAccount = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { accountId} = req.query;
  ...
  try {
    const updatedAccount = await accountService.updateAccount(accountId as string, account);
...
export const deleteAccount = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { accountId} = req.query;
  if (Array.isArray(accountId)) {
    return res.status(400).end('Bad request. Parameter cannot be an array.');
  }
