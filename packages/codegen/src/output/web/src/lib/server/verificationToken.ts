...
export const createVerificationToken = async (
  ...
  try {
    const verificationToken =
      await verificationTokenService.createVerificationToken(req.body);
...
export const getVerificationTokens = async (
  ...
  try {
    const verificationTokens =
      await verificationTokenService.getVerificationTokens({
        deletedAt: undefined,
      });
...
export const deleteVerificationToken = async (
  ...
  try {
    if (ALLOW_DELETE) {
      await verificationTokenService.deleteModelVerificationToken(
        verificationTokenId as string
      );
    }
