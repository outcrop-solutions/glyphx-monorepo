...
export const createProjectTemplate = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const projectTemplate = await projectTemplateService.createProjectTemplate(req.body);
...
export const getProjectTemplates = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const projectTemplates = await projectTemplateService.getProjectTemplates({deletedAt: undefined});
...
export const getProjectTemplate = async (req: NextApiRequest, res: NextApiResponse) => {
  const { projectTemplateId} = req.query;
  if (Array.isArray(projectTemplateId)) {
    return res.status(400).end('Bad request. Parameter cannot be an array.');
  }
  try {
    const projectTemplate = await projectTemplateService.getProjectTemplate(projectTemplateId as string);
    res.status(200).json({data: { projectTemplate }});
  } catch (error) {
    res.status(404).json({errors: {error: {msg: error.message} } });
  }
...
export const updateProjectTemplate = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  ...
  try {
    const updatedProjectTemplate = await projectTemplateService.updateProjectTemplate(projectTemplateId as string, projectTemplate);
...
export const deleteProjectTemplate = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  ...
  try {
    if (ALLOW_DELETE) {
      await projectTemplateService.deleteModelProjectTemplate(projectTemplateId as string);
    }
