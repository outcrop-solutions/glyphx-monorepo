...
export const createCustomerPayment = async (
  ...
  try {
    const customerPayment = await customerPaymentService.createCustomerPayment(
      req.body
    );
...
export const deleteCustomerPayment = async (
  ...
  try {
    if (ALLOW_DELETE) {
      await customerPaymentService.deleteModelCustomerPayment(
        customerPaymentId as string
      );
    }
