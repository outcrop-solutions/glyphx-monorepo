...
export const createModelConfig = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
...
export const getModelConfigs = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const modelConfigs = await modelConfigService.getModelConfigs({deletedAt: undefined});
...
export const getModelConfig = async (req: NextApiRequest, res: NextApiResponse) => {
  const { modelConfigId} = req.query;
  if (Array.isArray(modelConfigId)) {
    return res.status(400).end('Bad request. Parameter cannot be an array.');
  }
  try {
    const modelConfig = await modelConfigService.getModelConfig(modelConfigId as string);
    res.status(200).json({data: { modelConfig }});
  } catch (error) {
    res.status(404).json({errors: {error: {msg: error.message} } });
  }
...
export const updateModelConfig = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { modelConfigId} = req.query;
  ...
  try {
    const updatedModelConfig = await modelConfigService.updateModelConfig(modelConfigId as string, modelConfig);
...
export const deleteModelConfig = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { modelConfigId} = req.query;
  if (Array.isArray(modelConfigId)) {
    return res.status(400).end('Bad request. Parameter cannot be an array.');
  }
