...
export const createUserAgent = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
...
export const getUserAgents = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const userAgents = await userAgentService.getUserAgents({deletedAt: undefined});
...
export const getUserAgent = async (req: NextApiRequest, res: NextApiResponse) => {
  const { userAgentId} = req.query;
  if (Array.isArray(userAgentId)) {
    return res.status(400).end('Bad request. Parameter cannot be an array.');
  }
  try {
    const userAgent = await userAgentService.getUserAgent(userAgentId as string);
    res.status(200).json({data: { userAgent }});
  } catch (error) {
    res.status(404).json({errors: {error: {msg: error.message} } });
  }
...
export const updateUserAgent = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { userAgentId} = req.query;
  ...
  try {
    const updatedUserAgent = await userAgentService.updateUserAgent(userAgentId as string, userAgent);
...
export const deleteUserAgent = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { userAgentId} = req.query;
  if (Array.isArray(userAgentId)) {
    return res.status(400).end('Bad request. Parameter cannot be an array.');
  }
