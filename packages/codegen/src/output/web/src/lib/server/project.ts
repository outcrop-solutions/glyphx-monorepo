...
export const updateProject = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const { projectId} = req.query;
  ...
  try {
    const updatedProject = await projectService.updateProject(projectId as string, project);
