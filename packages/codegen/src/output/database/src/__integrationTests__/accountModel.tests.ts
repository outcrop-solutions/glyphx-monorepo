...
describe('#AccountModel', () => {
  context('test the crud functions of the account model', () => {
    ...
    it('add a new account ', async () => {
      ...
      assert.strictEqual(
        Object.keys(accountDocument)[1],
        Object.keys(accountInput)[1]
      );
    ...
    it('remove a account', async () => {
      assert.isOk(accountDocId);
      await accountModel.deleteAccountById(accountDocId);
      let errored = false;
      try {
        await accountModel.getAccountById(accountDocId);
      } catch (err) {
