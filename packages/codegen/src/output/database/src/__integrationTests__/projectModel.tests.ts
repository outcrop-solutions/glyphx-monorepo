...
describe('#ProjectModel', () => {
  context('test the crud functions of the project model', () => {
    ...
    it('add a new project ', async () => {
      ...
      assert.strictEqual(
        Object.keys(projectDocument)[1],
        Object.keys(projectInput)[1]
      );
    ...
    it('remove a project', async () => {
      assert.isOk(projectDocId);
      await projectModel.deleteProjectById(projectDocId);
      let errored = false;
      try {
        await projectModel.getProjectById(projectDocId);
      } catch (err) {
