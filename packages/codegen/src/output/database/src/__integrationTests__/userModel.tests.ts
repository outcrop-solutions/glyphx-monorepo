...
describe('#UserModel', () => {
  context('test the crud functions of the user model', () => {
    ...
    before(async () => {
      ...
      const savedCustomerPaymentDocument = await customerPaymentModel.create(
        [mocks.MOCK_CUSTOMERPAYMENT],
        {
          validateBeforeSave: false,
        }
      );
      customerPaymentId = savedCustomerPaymentDocument[0]
        ?._id as mongooseTypes.ObjectId;
    ...
    it('add a new user ', async () => {
      ...
      assert.strictEqual(
        Object.keys(userDocument)[1],
        Object.keys(userInput)[1]
      );
    ...
    it('remove a user', async () => {
      assert.isOk(userDocId);
      await userModel.deleteUserById(userDocId);
      let errored = false;
      try {
        await userModel.getUserById(userDocId);
      } catch (err) {
