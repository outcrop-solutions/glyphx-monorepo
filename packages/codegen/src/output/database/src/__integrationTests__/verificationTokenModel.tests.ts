...
describe('#VerificationTokenModel', () => {
  context('test the crud functions of the verificationToken model', () => {
    ...
    it('remove a verificationToken', async () => {
      assert.isOk(verificationTokenDocId);
      await verificationTokenModel.deleteVerificationTokenById(verificationTokenDocId);
      let errored = false;
      try {
        await verificationTokenModel.getVerificationTokenById(verificationTokenDocId);
      } catch (err) {
