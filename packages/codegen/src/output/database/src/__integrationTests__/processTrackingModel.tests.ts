...
describe('#ProcessTrackingModel', () => {
  context('test the crud functions of the processTracking model', () => {
    ...
    it('add a new processTracking ', async () => {
      const processTrackingInput = JSON.parse(
        JSON.stringify(mocks.MOCK_PROCESSTRACKING)
      );
      ...
      assert.strictEqual(
        Object.keys(processTrackingDocument)[1],
        Object.keys(processTrackingInput)[1]
      );
    ...
    it('retreive a processTracking', async () => {
      ...
      const processTracking =
        await processTrackingModel.getProcessTrackingById(processTrackingDocId);

      assert.isOk(processTracking);
      assert.strictEqual(
        processTracking._id?.toString(),
        processTrackingDocId.toString()
      );
    });
    ...
    it('remove a processTracking', async () => {
      assert.isOk(processTrackingDocId);
      await processTrackingModel.deleteProcessTrackingById(
        processTrackingDocId
      );
      let errored = false;
      try {
