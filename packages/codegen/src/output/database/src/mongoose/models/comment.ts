  ...
  async (input: ICommentCreateInput): Promise<databaseTypes.IComment> => {
    ...
    try {
      const [ 
        ...
        COMMENT_MODEL.validateAuthor(input.author),
                COMMENT_MODEL.validateState(input.state),
              ]);
...
SCHEMA.static(
  ...
  async (
    ...
    try {
      ...
      if (!commentDocument)
        throw new error.DataNotFoundError('A commentDocument with _id cannot be found', 'comment._id', commentId);
...
SCHEMA.static(
  ...
  async (
    ...
    if (
      !(await UserModel.userIdExists(authorId))
    ) {
      throw new error.InvalidArgumentError(
