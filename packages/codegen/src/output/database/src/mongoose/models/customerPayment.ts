...
SCHEMA.static(
  ...
  async (
    ...
    try {
      ...
      if (!customerPaymentDocument)
        throw new error.DataNotFoundError('A customerPaymentDocument with _id cannot be found', 'customerPayment._id', customerPaymentId);
...
SCHEMA.static(
  ...
  async (
    ...
    if (
      !(await UserModel.userIdExists(customerId))
    ) {
      throw new error.InvalidArgumentError(
