...
SCHEMA.static(
  ...
  async (customerPaymentId: mongooseTypes.ObjectId): Promise<boolean> => {
    let retval = false;
    try {
      const result = await CUSTOMERPAYMENT_MODEL.findById(customerPaymentId, [
        '_id',
      ]);
...
SCHEMA.static(
  ...
  async (customerPaymentIds: mongooseTypes.ObjectId[]): Promise<boolean> => {
    try {
      ...
      const foundIds = (await CUSTOMERPAYMENT_MODEL.find(
  ...
  async (
    ...
    try {
      const [customer] = await Promise.all([
        CUSTOMERPAYMENT_MODEL.validateCustomer(input.customer),
      ]);
      ...
      const customerPaymentDocument = (
        await CUSTOMERPAYMENT_MODEL.create([resolvedInput], {
...
SCHEMA.static(
  ...
  async (filter: Record<string, unknown> = {}, page = 0, itemsPerPage = 10) => {
    try {
      ...
      const customerPaymentDocuments = (await CUSTOMERPAYMENT_MODEL.find(
        filter,
        null,
        {
          skip: skip,
...
SCHEMA.static(
  ...
  async (
    ...
    await CUSTOMERPAYMENT_MODEL.updateCustomerPaymentWithFilter(
      {_id: customerPaymentId},
      customerPayment
    );
...
SCHEMA.static(
  ...
  async (customerPaymentId: mongooseTypes.ObjectId): Promise<void> => {
    try {
      const results = await CUSTOMERPAYMENT_MODEL.deleteOne({
        _id: customerPaymentId,
      });
...
SCHEMA.static(
  ...
  async (
    ...
    try {
      ...
      const customerPaymentDocument =
        await CUSTOMERPAYMENT_MODEL.findById(customerPaymentId);
      ...
      const reconciledId =
        await CUSTOMERPAYMENT_MODEL.validateCustomer(customer);
...
const CUSTOMERPAYMENT_MODEL = model<
  ICustomerPaymentDocument,
  ICustomerPaymentStaticMethods
