...
import {IModelConfigDocument, IModelConfigCreateInput, IModelConfigStaticMethods, IModelConfigMethods} from '../interfaces';
...
SCHEMA.static(
  ...
  async (modelConfigIds: mongooseTypes.ObjectId[]): Promise<boolean> => {
    try {
      ...
      const foundIds = (await MODELCONFIG_MODEL.find({_id: {$in: modelConfigIds}}, [
        '_id',
...
SCHEMA.static(
  'createModelConfig',
  async (input: IModelConfigCreateInput): Promise<databaseTypes.IModelConfig> => {
    ...
    try {
      ...
      const modelConfigDocument = (
        await MODELCONFIG_MODEL.create([resolvedInput], {validateBeforeSave: false})
      )[0];
...
const MODELCONFIG_MODEL = model<IModelConfigDocument, IModelConfigStaticMethods>(
  'modelConfig',
  SCHEMA
);
