...
describe('#mongoose/models/workspace', () => {
  ...
  context('validateUpdateObject', () => {
    ...
    it('will not throw an error when no unsafe fields are present', async () => {
      ...
      try {
        await WorkspaceModel.validateUpdateObject(mocks.MOCK_WORKSPACE as unknown as Omit<Partial<databaseTypes.IWorkspace>, '_id'>);
      } catch (err) {
        errored = true;
      }
    ...
    it('will fail when trying to update the _id', async () => {
      ...
      try {
        await WorkspaceModel.validateUpdateObject({...mocks.MOCK_WORKSPACE, _id: new mongoose.Types.ObjectId() } as unknown as Omit<Partial<databaseTypes.IWorkspace>, '_id'>);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
        errored = true;
      }
    ...
    it('will fail when trying to update the createdAt', async () => {
      ...
      try {
        await WorkspaceModel.validateUpdateObject({...mocks.MOCK_WORKSPACE, createdAt: new Date() } as unknown as Omit<Partial<databaseTypes.IWorkspace>, '_id'>);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
        errored = true;
      }
    ...
    it('will fail when trying to update the updatedAt', async () => {
      ...
      try {
        await WorkspaceModel.validateUpdateObject({...mocks.MOCK_WORKSPACE, updatedAt: new Date() }  as unknown as Omit<Partial<databaseTypes.IWorkspace>, '_id'>);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
        errored = true;
      }
  ...
  context('createWorkspace', () => {
    ...
    it('will create a workspace document', async () => {
      sandbox.replace(
        WorkspaceModel,
        'validateCreator',
        sandbox.stub().resolves(mocks.MOCK_WORKSPACE.creator)
      );
  ...
  context('getWorkspaceById', () => {
    ...
    it('will retreive a workspace document with the related fields populated', async () => {
      ...
      const doc = await WorkspaceModel.getWorkspaceById(
        mocks.MOCK_WORKSPACE._id as mongoose.Types.ObjectId
      );
