...
describe('#mongoose/models/comment', () => {
  ...
  context('validateUpdateObject', () => {
    ...
    it('will not throw an error when no unsafe fields are present', async () => {
      ...
      try {
        await CommentModel.validateUpdateObject(mocks.MOCK_COMMENT as unknown as Omit<Partial<databaseTypes.IComment>, '_id'>);
      } catch (err) {
        errored = true;
      }
    ...
    it('will fail when trying to update the _id', async () => {
      ...
      try {
        await CommentModel.validateUpdateObject({...mocks.MOCK_COMMENT, _id: new mongoose.Types.ObjectId() } as unknown as Omit<Partial<databaseTypes.IComment>, '_id'>);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
        errored = true;
      }
    ...
    it('will fail when trying to update the createdAt', async () => {
      ...
      try {
        await CommentModel.validateUpdateObject({...mocks.MOCK_COMMENT, createdAt: new Date() } as unknown as Omit<Partial<databaseTypes.IComment>, '_id'>);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
        errored = true;
      }
    ...
    it('will fail when trying to update the updatedAt', async () => {
      ...
      try {
        await CommentModel.validateUpdateObject({...mocks.MOCK_COMMENT, updatedAt: new Date() }  as unknown as Omit<Partial<databaseTypes.IComment>, '_id'>);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
        errored = true;
      }
  ...
  context('createComment', () => {
    ...
    it('will create a comment document', async () => {
      ...
      const commentDocument = await CommentModel.createComment(mocks.MOCK_COMMENT);
