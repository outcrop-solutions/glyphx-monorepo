...
SCHEMA.static(
  ...
  async (processTrackingId: mongooseTypes.ObjectId): Promise<boolean> => {
    let retval = false;
    try {
      const result = await PROCESSTRACKING_MODEL.findById(processTrackingId, [
        '_id',
      ]);
...
SCHEMA.static(
  ...
  async (processTrackingIds: mongooseTypes.ObjectId[]): Promise<boolean> => {
    try {
      ...
      const foundIds = (await PROCESSTRACKING_MODEL.find(
  ...
  async (
    ...
    try {
      ...
      const processTrackingDocument = (
        await PROCESSTRACKING_MODEL.create([resolvedInput], {
...
SCHEMA.static(
  ...
  async (processTrackingId: mongooseTypes.ObjectId) => {
    try {
      const processTrackingDocument = (await PROCESSTRACKING_MODEL.findById(
        processTrackingId
      )
      ...
      processTrackingDocument.processMessages?.forEach(
...
SCHEMA.static(
  ...
  async (filter: Record<string, unknown> = {}, page = 0, itemsPerPage = 10) => {
    try {
      ...
      const processTrackingDocuments = (await PROCESSTRACKING_MODEL.find(
        filter,
        null,
        {
          skip: skip,
      ...
      processTrackingDocuments.forEach((doc: any) => {
        ...
        (doc as any).processMessages?.forEach(
          (m: any) => delete (m as any)['__v']
        );
      });
...
SCHEMA.static(
  ...
  async (
    ...
    await PROCESSTRACKING_MODEL.updateProcessTrackingWithFilter(
      {_id: processTrackingId},
      processTracking
    );
    return await PROCESSTRACKING_MODEL.getProcessTrackingById(
      processTrackingId
    );
  }
);
...
SCHEMA.static(
  ...
  async (processTrackingId: mongooseTypes.ObjectId): Promise<void> => {
    try {
      const results = await PROCESSTRACKING_MODEL.deleteOne({
        _id: processTrackingId,
      });
...
const PROCESSTRACKING_MODEL = model<
  IProcessTrackingDocument,
  IProcessTrackingStaticMethods
