...
describe('#WebhookModel', () => {
  context('test the crud functions of the webhook model', () => {
    ...
    it('add a new webhook ', async () => {
      ...
      assert.strictEqual(
        Object.keys(webhookDocument)[1],
        Object.keys(webhookInput)[1]
      );
    ...
    it('remove a webhook', async () => {
      assert.isOk(webhookDocId);
      await webhookModel.deleteWebhookById(webhookDocId);
      let errored = false;
      try {
        await webhookModel.getWebhookById(webhookDocId);
      } catch (err) {
