...
describe('#CustomerPaymentModel', () => {
  context('test the crud functions of the customerPayment model', () => {
    ...
    before(async () => {
      ...
      const savedCustomerDocument = await customerModel.create(
        [mocks.MOCK_USER],
        {
    ...
    it('add a new customerPayment ', async () => {
      const customerPaymentInput = JSON.parse(
        JSON.stringify(mocks.MOCK_CUSTOMERPAYMENT)
      );
      ...
      assert.strictEqual(
        Object.keys(customerPaymentDocument)[1],
        Object.keys(customerPaymentInput)[1]
      );
    ...
    it('remove a customerPayment', async () => {
      assert.isOk(customerPaymentDocId);
      await customerPaymentModel.deleteCustomerPaymentById(
        customerPaymentDocId
      );
      let errored = false;
      try {
        await customerPaymentModel.getCustomerPaymentById(customerPaymentDocId);
      } catch (err) {
