...
export class StateService {
  public static async getState(
    ...
    try {
      ...
      const state =
        await mongoDbConnection.models.StateModel.getStateById(id);
      return state;
    } catch (err: any) {
    ...
    try {
      const id =
        stateId instanceof mongooseTypes.ObjectId
          ...
          : new mongooseTypes.ObjectId(stateId);
      const state =
        await mongoDbConnection.models.StateModel.updateStateById(id, {
          ...data
        });
      return state;
    } catch (err: any) {
    ...
    try {
      const id =
        stateId instanceof mongooseTypes.ObjectId
          ...
          : new mongooseTypes.ObjectId(stateId);
      const updatedState =
        await mongoDbConnection.models.StateModel.addProject(id, project);
