...
export class ProcessTrackingService {
  public static async getProcessTracking(
    ...
    try {
      ...
      const processTracking =
        await mongoDbConnection.models.ProcessTrackingModel.getProcessTrackingById(
          id
        );
  ...
  public static async getProcessTrackings(
    ...
    try {
      const processTrackings =
        await mongoDbConnection.models.ProcessTrackingModel.queryProcessTrackings(
          filter
        );
  ...
  public static async createProcessTracking(
    ...
    try {
      ...
      const processTracking =
        await mongoDbConnection.models.ProcessTrackingModel.createProcessTracking(
          data
        );
  ...
  public static async updateProcessTracking(
    processTrackingId: mongooseTypes.ObjectId | string,
    data: Partial<
      Omit<databaseTypes.IProcessTracking, '_id' | 'createdAt' | 'updatedAt'>
    >
  ): Promise<databaseTypes.IProcessTracking> {
    try {
      const id =
        processTrackingId instanceof mongooseTypes.ObjectId
          ...
          : new mongooseTypes.ObjectId(processTrackingId);
      const processTracking =
        await mongoDbConnection.models.ProcessTrackingModel.updateProcessTrackingById(
          id,
          {
            ...data,
          }
