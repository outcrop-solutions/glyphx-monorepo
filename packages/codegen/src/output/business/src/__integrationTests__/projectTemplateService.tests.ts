...
describe('#ProjectTemplateService', () => {
  context('test the functions of the projectTemplate service', () => {
    ...
    before(async () => {
      ...
      projectTemplateId = projectTemplateDocument._id as unknown as mongooseTypes.ObjectId;
    ...
    it('will update our projectTemplate', async () => {
      ...
      const updatedProjectTemplate = await projectTemplateService.updateProjectTemplate(projectTemplateId, {
        deletedAt: new Date()
      });
      assert.isOk(updatedProjectTemplate.deletedAt);

      const savedProjectTemplate = await projectTemplateService.getProjectTemplate(projectTemplateId);
