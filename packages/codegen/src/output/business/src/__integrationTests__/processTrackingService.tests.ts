...
describe('#ProcessTrackingService', () => {
  context('test the functions of the processTracking service', () => {
    ...
    before(async () => {
      ...
      const processTrackingDocument = await processTrackingModel.createProcessTracking(
        // @ts-ignore
        mocks.MOCK_PROCESSTRACKING as unknown as databaseTypes.IProcessTracking
      );
      processTrackingId = processTrackingDocument._id as unknown as mongooseTypes.ObjectId;
    ...
    it('will update our processTracking', async () => {
      ...
      const updatedProcessTracking = await processTrackingService.updateProcessTracking(processTrackingId, {
        deletedAt: new Date()
      });
      assert.isOk(updatedProcessTracking.deletedAt);

      const savedProcessTracking = await processTrackingService.getProcessTracking(processTrackingId);
