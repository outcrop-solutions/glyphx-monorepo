...
import { InvitedMemberModel} from './invitedMember'
import { CreatedWorkspaceModel} from './createdWorkspace'
  ...
  async (input: IUserCreateInput): Promise<databaseTypes.IUser> => {
    ...
    try {
      const [ 
        ...
        USER_MODEL.validateAccounts(input.workspace ?? []),
        USER_MODEL.validateSessions(input.workspace ?? []),
        USER_MODEL.validateMemberships(input.workspace ?? []),
        USER_MODEL.validateInvitedMembers(input.workspace ?? []),
        USER_MODEL.validateCreatedWorkspaces(input.workspace ?? []),
        USER_MODEL.validateProjects(input.workspace ?? []),
        USER_MODEL.validateWebhooks(input.workspace ?? []),
      ]);
...
SCHEMA.static(
    ...
    try {
      ...
      const reconciledIds = await USER_MODEL.validateAccount(accounts);
      let dirty = false;
      reconciledIds.forEach((p: any) => {
        if (
          ...
          dirty = true;
          userDocument.accounts.push(
            p as unknown as databaseTypes.IAccount
          );
        }
      });
...
SCHEMA.static(
    ...
    try {
      ...
      const reconciledIds = await USER_MODEL.validateSession(sessions);
      let dirty = false;
      reconciledIds.forEach((p: any) => {
        if (
          ...
          userDocument.sessions.push(
            p as unknown as databaseTypes.ISession
          );
        }
      });
...
SCHEMA.static(
    ...
    try {
      ...
      const reconciledIds = await USER_MODEL.validateMember(members);
      let dirty = false;
      reconciledIds.forEach((p: any) => {
        if (
...
SCHEMA.static(
    ...
    try {
      ...
      const reconciledIds = await USER_MODEL.validateWorkspace(workspaces);
      let dirty = false;
      reconciledIds.forEach((p: any) => {
        if (
          !userDocument.workspaces.find(
            (id: any) => id.toString() === p.toString()
          )
        ) {
          dirty = true;
          userDocument.workspaces.push(
...
SCHEMA.static(
    ...
    try {
      ...
      const reconciledIds = await USER_MODEL.validateProject(projects);
      let dirty = false;
      reconciledIds.forEach((p: any) => {
        if (
...
SCHEMA.static(
    ...
    try {
      ...
      const reconciledIds = await USER_MODEL.validateWebhook(webhooks);
      let dirty = false;
      reconciledIds.forEach((p: any) => {
        if (
          ...
          dirty = true;
          userDocument.webhooks.push(
            p as unknown as databaseTypes.IWebhook
          );
        }
      });
