...
import {ISessionDocument, ISessionStaticMethods, ISessionMethods} from '../interfaces';
...
SCHEMA.static(addIuser: async (
  ...
  ): Promise<databaseTypes.IIsession> => {
    try {
      if (!IUsers.length)
      ...
      const document = await IsessionModel.findById(id);
      if (!document)
      ...
      const reconciledIds = await IsessionModel.validateIuser(IUsers);
      let dirty = false;
      ...
      return await IsessionModel.getById(id);
    } catch (err) {
      if (
