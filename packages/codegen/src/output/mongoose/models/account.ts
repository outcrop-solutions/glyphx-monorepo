...
const SCHEMA = new Schema<IAccountDocument, IAccountStaticMethods, IAccountMethods>({
  ...
  provider: {
    type: String,
    required: false,
    enum: databaseTypes.constants.Provider,
  },
  providerAccountId: {
    type: String,
    required: true,
  ...
  token_type: {
    type: String,
    required: false,
    enum: databaseTypes.constants.TokenType,
  },
  scope: {
    type: String,
    required: true,
  ...
  session_state: {
    type: String,
    required: false,
    enum: databaseTypes.constants.SessionState,
  },
  oauth_token_secret: {
    type: String,
    required: true,
...
SCHEMA.static('allAccountIdsExist', async (accountIds: mongooseTypes.ObjectId[]): Promise<boolean> => {
  try {
    ...
    const foundIds = (await ACCOUNT_MODEL.find({ _id: { $in: accountIds } }, ['_id'])) as {
