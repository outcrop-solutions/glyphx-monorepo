...
describe('#services/processTracking', () => {
  ...
  context('createProcessTracking', () => {
    it('will create a ProcessTracking', async () => {
      ...
      const processResultId = new mongooseTypes.ObjectId();
  ...
  context('getProcessTracking', () => {
    it('should get a processTracking by id', async () => {
      const processTrackingId = new mongooseTypes.ObjectId().toString();
      ...
      assert.strictEqual(processTracking?._id?.toString(), processTrackingId.toString());
    ...
    it('should get a processTracking by id when id is a string', async () => {
      ...
      const processTracking = await processTrackingService.getProcessTracking(processTrackingId.toString());
      assert.isOk(processTracking);
      assert.strictEqual(processTracking?._id?.toString(), processTrackingId.toString());
    ...
    it('will log the failure and return null if the processTracking cannot be found', async () => {
      ...
      const err = new error.DataNotFoundError(errMessage, 'processTrackingId', processTrackingId);
      const getProcessTrackingFromModelStub = sandbox.stub();
    ...
    it('will log the failure and throw a DatabaseService when the underlying model call fails', async () => {
      ...
      const err = new error.DatabaseOperationError(errMessage, 'mongoDb', 'getProcessTrackingById');
      const getProcessTrackingFromModelStub = sandbox.stub();
      getProcessTrackingFromModelStub.rejects(err);
      sandbox.replace(
        dbConnection.models.ProcessTrackingModel,
  ...
  context('getProcessTrackings', () => {
    it('should get processTrackings by filter', async () => {
      ...
      const processTrackings = await processTrackingService.getProcessTrackings(processTrackingFilter);
      assert.isOk(processTrackings![0]);
      assert.strictEqual(processTrackings![0]._id?.toString(), processTrackingId.toString());
    ...
    it('will log the failure and return null if the processTrackings cannot be found', async () => {
      ...
      const err = new error.DataNotFoundError(errMessage, 'name', processTrackingFilter);
      const getProcessTrackingFromModelStub = sandbox.stub();
      getProcessTrackingFromModelStub.rejects(err);
      sandbox.replace(
        dbConnection.models.ProcessTrackingModel,
      ...
      const processTracking = await processTrackingService.getProcessTrackings(processTrackingFilter);
      assert.notOk(processTracking);
    ...
    it('will log the failure and throw a DatabaseService when the underlying model call fails', async () => {
      ...
      const processTrackingFilter = {name: processTrackingName};
      const errMessage = 'Something Bad has happened';
      const err = new error.DatabaseOperationError(errMessage, 'mongoDb', 'getProcessTrackingByEmail');
      const getProcessTrackingFromModelStub = sandbox.stub();
      getProcessTrackingFromModelStub.rejects(err);
      sandbox.replace(
        dbConnection.models.ProcessTrackingModel,
  ...
  context('updateProcessTracking', () => {
    it('will update a processTracking', async () => {
      ...
      const processTracking = await processTrackingService.updateProcessTracking(processTrackingId, {
        deletedAt: new Date(),
      });
      assert.isOk(processTracking);
      assert.strictEqual(processTracking.id, 'id');
    ...
    it('will update a processTracking when the id is a string', async () => {
      ...
      const processTracking = await processTrackingService.updateProcessTracking(processTrackingId.toString(), {
        deletedAt: new Date(),
      });
      assert.isOk(processTracking);
      assert.strictEqual(processTracking.id, 'id');
    ...
    it('will publish and rethrow an InvalidArgumentError when processTracking model throws it ', async () => {
      ...
      let errored = false;
      try {
        await processTrackingService.updateProcessTracking(processTrackingId, {deletedAt: new Date()});
      } catch (e) {
        assert.instanceOf(e, error.InvalidArgumentError);
        errored = true;
      }
    ...
    it('will publish and throw an DataServiceError when processTracking model throws a DataOperationError ', async () => {
      const processTrackingId = new mongooseTypes.ObjectId().toString();
      const errMessage = 'A DataOperationError has occurred';
      const err = new error.DatabaseOperationError(errMessage, 'mongodDb', 'updateProcessTrackingById');
      const updateProcessTrackingFromModelStub = sandbox.stub();
      updateProcessTrackingFromModelStub.rejects(err);
      sandbox.replace(
        dbConnection.models.ProcessTrackingModel,
