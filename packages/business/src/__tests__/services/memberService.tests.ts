...
describe('#services/member', () => {
  ...
  context('createMember', () => {
    it('will create a Member', async () => {
      ...
      sandbox.replace(dbConnection.models.MemberModel, 'createMember', createMemberFromModelStub);
  ...
  context('getMember', () => {
    it('should get a member by id', async () => {
      const memberId = new mongooseTypes.ObjectId().toString();

      const getMemberFromModelStub = sandbox.stub();
      getMemberFromModelStub.resolves({
        _id: memberId,
      } as unknown as databaseTypes.IMember);
      sandbox.replace(dbConnection.models.MemberModel, 'getMemberById', getMemberFromModelStub);
    ...
    it('will log the failure and throw a DatabaseService when the underlying model call fails', async () => {
      ...
      const err = new error.DatabaseOperationError(errMessage, 'mongoDb', 'getMemberById');
      const getMemberFromModelStub = sandbox.stub();
      getMemberFromModelStub.rejects(err);
      sandbox.replace(dbConnection.models.MemberModel, 'getMemberById', getMemberFromModelStub);
      function fakePublish() {
  ...
  context('getMembers', () => {
    ...
    it('will log the failure and return null if the members cannot be found', async () => {
      ...
      const getMemberFromModelStub = sandbox.stub();
      getMemberFromModelStub.rejects(err);
      sandbox.replace(dbConnection.models.MemberModel, 'queryMembers', getMemberFromModelStub);
      function fakePublish() {
        //@ts-ignore
        assert.instanceOf(this, error.DataNotFoundError);
    ...
    it('will log the failure and throw a DatabaseService when the underlying model call fails', async () => {
      ...
      const memberFilter = {name: memberName};
      const errMessage = 'Something Bad has happened';
      const err = new error.DatabaseOperationError(errMessage, 'mongoDb', 'getMemberByEmail');
      const getMemberFromModelStub = sandbox.stub();
      getMemberFromModelStub.rejects(err);
      sandbox.replace(dbConnection.models.MemberModel, 'queryMembers', getMemberFromModelStub);
      function fakePublish() {
  ...
  context('updateMember', () => {
    it('will update a member', async () => {
      ...
      sandbox.replace(dbConnection.models.MemberModel, 'updateMemberById', updateMemberFromModelStub);
    ...
    it('will publish and throw an DataServiceError when member model throws a DataOperationError ', async () => {
      const memberId = new mongooseTypes.ObjectId().toString();
      const errMessage = 'A DataOperationError has occurred';
      const err = new error.DatabaseOperationError(errMessage, 'mongodDb', 'updateMemberById');
      const updateMemberFromModelStub = sandbox.stub();
      updateMemberFromModelStub.rejects(err);
      sandbox.replace(dbConnection.models.MemberModel, 'updateMemberById', updateMemberFromModelStub);
