...
describe('#services/activityLog', () => {
  ...
  context('getActivityLog', () => {
    it('should get a activityLog by id', async () => {
      const activityLogId = new mongooseTypes.ObjectId().toString();
    ...
    it('should get a activityLog by id when id is a string', async () => {
      ...
      const activityLog = await activityLogService.getActivityLog(activityLogId.toString());
      assert.isOk(activityLog);
      assert.strictEqual(activityLog?._id?.toString(), activityLogId.toString());
  ...
  context('getActivityLogs', () => {
    it('should get activityLogs by filter', async () => {
      ...
      const activityLogs = await activityLogService.getActivityLogs(activityLogFilter);
      assert.isOk(activityLogs![0]);
      assert.strictEqual(activityLogs![0]._id?.toString(), activityLogId.toString());
    ...
    it('will log the failure and return null if the activityLogs cannot be found', async () => {
      ...
      const activityLog = await activityLogService.getActivityLogs(activityLogFilter);
      assert.notOk(activityLog);
  ...
  context('updateActivityLog', () => {
    it('will update a activityLog', async () => {
      ...
      const activityLog = await activityLogService.updateActivityLog(activityLogId, {
        deletedAt: new Date(),
      });
      assert.isOk(activityLog);
      assert.strictEqual(activityLog.id, 'id');
    ...
    it('will update a activityLog when the id is a string', async () => {
     const activityLogId = new mongooseTypes.ObjectId();
      ...
      const activityLog = await activityLogService.updateActivityLog(activityLogId.toString(), {
        deletedAt: new Date(),
      });
      assert.isOk(activityLog);
      assert.strictEqual(activityLog.id, 'id');
    ...
    it('will publish and rethrow an InvalidArgumentError when activityLog model throws it', async () => {
      ...
      let errored = false;
      try {
        await activityLogService.updateActivityLog(activityLogId, {deletedAt: new Date()});
      } catch (e) {
        assert.instanceOf(e, error.InvalidArgumentError);
        errored = true;
      }
