...
export class AiStreamManager {
  ...
  private defaultModels: Record<
    ...
    | aiTypes.ANTHROPIC_MODELS
    | aiTypes.OPENAI_MODELS
    | aiTypes.LANGCHAIN_MODELS
    | aiTypes.HF_MODELS
  >;
  ...
  async getResponse(
    ...
    switch (this.streamSource) {
      case 'ANTHROPIC':
        return await this.getAnthropicStream(
          prompt,
          model as aiTypes.ANTHROPIC_MODELS
        );
      case 'HUGGINGFACE':
        return await this.getHuggingFaceStream(
          prompt,
          model as aiTypes.HF_MODELS
        );
      case 'LANGCHAIN':
        return await this.getLangChainStream(prompt);
      case 'OPENAI':
        return await this.getOpenAIStream(
          prompt,
          model as aiTypes.OPENAI_MODELS
        );
    }
  }
  ...
  private async getAnthropicStream(
    prompt: string,
    model: aiTypes.ANTHROPIC_MODELS
  ) {
