...
describe('#aws/util/ResultSetConverter', () => {
  context('fromResultSet', () => {
    it('Should convert the resultSet to a POJSO', () => {
      const convertedData = ResultSetConverter.fromResultset(ResultSetMock.ResultSet);
      assert.strictEqual(convertedData.length, ResultSetMock.ResultSet.Rows.length - 1);

      const randomIdx = Math.floor(Math.random() * (ResultSetMock.ResultSet.Rows.length - 1)) + 1;
      ...
      assert.strictEqual(convertedRow.col1, Number(inputRow.Data[0].VarCharValue));
      ...
      assert.strictEqual(convertedRow.col4, Number(inputRow.Data[3].VarCharValue));
    });
    it('Should convert the resultSet with undefined numbers to a POJSO', () => {
      ...
      const convertedData = ResultSetConverter.fromResultset(mockDataCopy.ResultSet);
      assert.strictEqual(convertedData.length, ResultSetMock.ResultSet.Rows.length - 1);
