...
SCHEMA.static('userAgentIdExists', async (userAgentId: mongooseTypes.ObjectId): Promise<boolean> => {
  let retval = false;
  try {
...
SCHEMA.static('allUserAgentIdsExist', async (userAgentIds: mongooseTypes.ObjectId[]): Promise<boolean> => {
  try {
    const notFoundIds: mongooseTypes.ObjectId[] = [];
    const foundIds = (await USERAGENT_MODEL.find({_id: {$in: userAgentIds}}, ['_id'])) as {
...
SCHEMA.static(
  ...
  async (userAgent: Omit<Partial<databaseTypes.IUserAgent>, '_id'>): Promise<void> => {
    ...
    if ((userAgent as Record<string, unknown>)['_id'])
      throw new error.InvalidOperationError('The userAgent._id is immutable and cannot be changed', {
...
// CREATE
SCHEMA.static('createUserAgent', async (input: IUserAgentCreateInput): Promise<databaseTypes.IUserAgent> => {
  ...
  try {
    ...
    const userAgentDocument = (await USERAGENT_MODEL.create([resolvedInput], {validateBeforeSave: false}))[0];
  ...
  if (id) return await USERAGENT_MODEL.getUserAgentById(id.toString());
  else
...
// READ
SCHEMA.static('getUserAgentById', async (userAgentId: string) => {
  try {
    const userAgentDocument = (await USERAGENT_MODEL.findById(userAgentId).lean()) as databaseTypes.IUserAgent;
    ...
    return format.toJS(userAgentDocument);
  } catch (err) {
    if (err instanceof error.DataNotFoundError) throw err;
    else
      throw new error.DatabaseOperationError(
...
SCHEMA.static('queryUserAgents', async (filter: Record<string, unknown> = {}, page = 0, itemsPerPage = 10) => {
  try {
    ...
    const format = new DBFormatter();
    const userAgents = userAgentDocuments?.map((doc: any) => {
      return format.toJS(doc);
    });

    const retval: IQueryResult<databaseTypes.IUserAgent> = {
      results: userAgents as unknown as databaseTypes.IUserAgent[],
      numberOfItems: count,
      page: page,
...
SCHEMA.static(
  ...
  async (filter: Record<string, unknown>, userAgent: Omit<Partial<databaseTypes.IUserAgent>, '_id'>): Promise<void> => {
    try {
      await USERAGENT_MODEL.validateUpdateObject(userAgent);
      const updateDate = new Date();
      const transformedObject: Partial<IUserAgentDocument> & Record<string, unknown> = {updatedAt: updateDate};
      const updateResult = await USERAGENT_MODEL.updateOne(filter, transformedObject);
...
// DELETE
SCHEMA.static('deleteUserAgentById', async (userAgentId: string): Promise<void> => {
  try {
...
const USERAGENT_MODEL = model<IUserAgentDocument, IUserAgentStaticMethods>('userAgent', SCHEMA);
