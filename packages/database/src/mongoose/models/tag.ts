...
SCHEMA.static(
  'createTag',
  async (
    input: Omit<databaseTypes.ITag, '_id'>
...
SCHEMA.static(
  ...
  (tag: Omit<Partial<databaseTypes.ITag>, '_id'>): void => {
    if (tag.createdAt) {
    ...
    if ((tag as Record<string, unknown>)['_id']) {
      throw new error.InvalidOperationError(
        'The _id on a tag is immutable and cannot be changed',
...
SCHEMA.static(
  ...
  async (
    ...
    try {
      ...
      if (!tagDocument)
        throw new error.DataNotFoundError(
          `A Tag Document with _id : ${tagId} cannot be found`,
          'tag._id',
          tagId
        );
...
SCHEMA.static(
  ...
  async (
    ...
    try {
      ...
      if (!tagDocument)
        throw new error.DataNotFoundError(
          `An Tag Document with _id : ${tagId} cannot be found`,
          'tag ._id',
          tagId
        );
      ...
      reconciledIds.forEach(p => {
        if (
          !tagDocument.workspaces.find(
            (wId: any) => wId.toString() === p.toString()
          )
        ) {
          dirty = true;
      ...
      reconciledIds.forEach(p => {
        if (
          !tagDocument.templates.find(
            (tagId: any) => tagId.toString() === p.toString()
          )
        ) {
          dirty = true;
