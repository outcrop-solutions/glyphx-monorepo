...
SCHEMA.static('modelConfigIdExists', async (modelConfigId: mongooseTypes.ObjectId): Promise<boolean> => {
  let retval = false;
  try {
...
SCHEMA.static('allModelConfigIdsExist', async (modelConfigIds: mongooseTypes.ObjectId[]): Promise<boolean> => {
  try {
    const notFoundIds: mongooseTypes.ObjectId[] = [];
    const foundIds = (await MODELCONFIG_MODEL.find({_id: {$in: modelConfigIds}}, ['_id'])) as {
...
SCHEMA.static(
  ...
  async (modelConfig: Omit<Partial<databaseTypes.IModelConfig>, '_id'>): Promise<void> => {
    ...
    if ((modelConfig as Record<string, unknown>)['_id'])
      throw new error.InvalidOperationError('The modelConfig._id is immutable and cannot be changed', {
...
// CREATE
SCHEMA.static('createModelConfig', async (input: IModelConfigCreateInput): Promise<databaseTypes.IModelConfig> => {
...
SCHEMA.static('queryModelConfigs', async (filter: Record<string, unknown> = {}, page = 0, itemsPerPage = 10) => {
  try {
...
const MODELCONFIG_MODEL = model<IModelConfigDocument, IModelConfigStaticMethods>('modelConfig', SCHEMA);
