...
SCHEMA.static('allActivityLogIdsExist', async (activityLogIds: mongooseTypes.ObjectId[]): Promise<boolean> => {
  try {
    ...
    const foundIds = (await ACTIVITYLOG_MODEL.find({_id: {$in: activityLogIds}}, ['_id'])) as {
...
SCHEMA.static(
  ...
  async (activityLog: Omit<Partial<databaseTypes.IActivityLog>, '_id'>): Promise<void> => {
    ...
    if (activityLog.actor)
      tasks.push(idValidator(activityLog.actor._id as mongooseTypes.ObjectId, 'User', UserModel.userIdExists));
    if (activityLog.workspace)
      tasks.push(
        idValidator(activityLog.workspace._id as mongooseTypes.ObjectId, 'Workspace', WorkspaceModel.workspaceIdExists)
      );
    if (activityLog.project)
      tasks.push(
        idValidator(activityLog.project._id as mongooseTypes.ObjectId, 'Project', ProjectModel.projectIdExists)
      );
    if (activityLog.userAgent)
      tasks.push(
        idValidator(activityLog.userAgent._id as mongooseTypes.ObjectId, 'UserAgent', UserAgentModel.userAgentIdExists)
      );
    ...
    if ((activityLog as Record<string, unknown>)['_id'])
      throw new error.InvalidOperationError('The activityLog._id is immutable and cannot be changed', {
...
SCHEMA.static('createActivityLog', async (input: IActivityLogCreateInput): Promise<databaseTypes.IActivityLog> => {
  ...
  try {
    ...
    const activityLogDocument = (await ACTIVITYLOG_MODEL.create([resolvedInput], {validateBeforeSave: false}))[0];
  ...
  if (id) return await ACTIVITYLOG_MODEL.getActivityLogById(id.toString());
  else
...
SCHEMA.static('queryActivityLogs', async (filter: Record<string, unknown> = {}, page = 0, itemsPerPage = 10) => {
  try {
    ...
    const format = new DBFormatter();
    const activityLogs = activityLogDocuments?.map((doc: any) => {
      return format.toJS(doc);
    });

    const retval: IQueryResult<databaseTypes.IActivityLog> = {
      results: activityLogs as unknown as databaseTypes.IActivityLog[],
      numberOfItems: count,
      page: page,
...
SCHEMA.static(
  ...
  async (
    ...
    try {
      ...
      const transformedObject: Partial<IActivityLogDocument> & Record<string, unknown> = {updatedAt: updateDate};
      ...
      const updateResult = await ACTIVITYLOG_MODEL.updateOne(filter, transformedObject);
...
// DELETE
SCHEMA.static('deleteActivityLogById', async (activityLogId: string): Promise<void> => {
  try {
...
SCHEMA.static(
  'addActor',
  async (activityLogId: string, actor: databaseTypes.IUser | string): Promise<databaseTypes.IActivityLog> => {
    try {
      if (!actor) throw new error.InvalidArgumentError('You must supply at least one id', 'actor', actor);
      const activityLogDocument = await ACTIVITYLOG_MODEL.findById(activityLogId);
...
SCHEMA.static('removeActor', async (activityLogId: string): Promise<databaseTypes.IActivityLog> => {
  try {
    const activityLogDocument = await ACTIVITYLOG_MODEL.findById(activityLogId);
    if (!activityLogDocument)
      throw new error.DataNotFoundError(
...
SCHEMA.static('validateActor', async (input: databaseTypes.IUser | string): Promise<mongooseTypes.ObjectId> => {
  const actorId = typeof input === 'string' ? new mongooseTypes.ObjectId(input) : new mongooseTypes.ObjectId(input.id);

  if (!(await UserModel.userIdExists(actorId))) {
    throw new error.InvalidArgumentError(`The actor: ${actorId} does not exist`, 'actorId', actorId);
  }
  return actorId;
});
...
SCHEMA.static(
  'addWorkspace',
  async (activityLogId: string, workspace: databaseTypes.IWorkspace | string): Promise<databaseTypes.IActivityLog> => {
    try {
      ...
      if (activityLogDocument.workspace?.toString() !== reconciledId.toString()) {
        const reconciledId = await ACTIVITYLOG_MODEL.validateWorkspace(workspace);
...
SCHEMA.static('removeWorkspace', async (activityLogId: string): Promise<databaseTypes.IActivityLog> => {
  try {
    const activityLogDocument = await ACTIVITYLOG_MODEL.findById(activityLogId);
    if (!activityLogDocument)
      throw new error.DataNotFoundError(
...
SCHEMA.static(
  'addProject',
  async (activityLogId: string, project: databaseTypes.IProject | string): Promise<databaseTypes.IActivityLog> => {
    try {
...
SCHEMA.static('removeProject', async (activityLogId: string): Promise<databaseTypes.IActivityLog> => {
  try {
    const activityLogDocument = await ACTIVITYLOG_MODEL.findById(activityLogId);
    if (!activityLogDocument)
      throw new error.DataNotFoundError(
...
SCHEMA.static(
  'addUserAgent',
  async (activityLogId: string, userAgent: databaseTypes.IUserAgent | string): Promise<databaseTypes.IActivityLog> => {
    try {
      if (!userAgent) throw new error.InvalidArgumentError('You must supply at least one id', 'userAgent', userAgent);
      const activityLogDocument = await ACTIVITYLOG_MODEL.findById(activityLogId);
      ...
      if (activityLogDocument.userAgent?.toString() !== reconciledId.toString()) {
        const reconciledId = await ACTIVITYLOG_MODEL.validateUserAgent(userAgent);
...
SCHEMA.static('removeUserAgent', async (activityLogId: string): Promise<databaseTypes.IActivityLog> => {
  try {
    const activityLogDocument = await ACTIVITYLOG_MODEL.findById(activityLogId);
    if (!activityLogDocument)
      throw new error.DataNotFoundError(
...
SCHEMA.static(
  'validateUserAgent',
  async (input: databaseTypes.IUserAgent | string): Promise<mongooseTypes.ObjectId> => {
    const userAgentId =
      typeof input === 'string' ? new mongooseTypes.ObjectId(input) : new mongooseTypes.ObjectId(input.id);

    if (!(await UserAgentModel.userAgentIdExists(userAgentId))) {
      throw new error.InvalidArgumentError(`The userAgent: ${userAgentId} does not exist`, 'userAgentId', userAgentId);
    }
    return userAgentId;
  }
);
...
const ACTIVITYLOG_MODEL = model<IActivityLogDocument, IActivityLogStaticMethods>('activityLog', SCHEMA);
