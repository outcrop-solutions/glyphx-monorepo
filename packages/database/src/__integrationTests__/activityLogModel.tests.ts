...
describe('#ActivityLogModel', () => {
  context('test the crud functions of the activityLog model', () => {
    ...
    it('retreive a activityLog', async () => {
      ...
      const activityLog = await activityLogModel.getActivityLogById(activityLogDocId);

      assert.isOk(activityLog);
      assert.strictEqual(activityLog._id?.toString(), activityLogDocId.toString());
    });
    ...
    it('remove a activityLog', async () => {
      assert.isOk(activityLogDocId);
      await activityLogModel.deleteActivityLogById(activityLogDocId.toString());
      let errored = false;
      try {
        await activityLogModel.getActivityLogById(activityLogDocId.toString());
      } catch (err) {
