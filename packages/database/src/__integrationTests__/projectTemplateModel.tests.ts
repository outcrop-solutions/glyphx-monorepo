...
describe('#ProjectTemplateModel', () => {
  context('test the crud functions of the projectTemplate model', () => {
    ...
    it('retreive a projectTemplate', async () => {
      ...
      const projectTemplate = await projectTemplateModel.getProjectTemplateById(projectTemplateDocId);

      assert.isOk(projectTemplate);
      assert.strictEqual(projectTemplate._id?.toString(), projectTemplateDocId.toString());
    });
    ...
    it('remove a projectTemplate', async () => {
      assert.isOk(projectTemplateDocId);
      await projectTemplateModel.deleteProjectTemplateById(projectTemplateDocId.toString());
      let errored = false;
      try {
        await projectTemplateModel.getProjectTemplateById(projectTemplateDocId.toString());
      } catch (err) {
