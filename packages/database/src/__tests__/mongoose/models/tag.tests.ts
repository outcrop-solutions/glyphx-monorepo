...
const MOCK_TAG: databaseTypes.ITag = {
  ...
  value: 'testTag',
  workspaces: [
    {_id: new mongoose.Types.ObjectId()} as unknown as databaseTypes.IWorkspace,
  ],
  projects: [
...
describe('#mongoose/models/tag', () => {
  ...
  context('createTag', () => {
    ...
    it('will create a tag document', async () => {
      sandbox.replace(
        TagModel,
        'validateWorkspaces',
        sandbox.stub().resolves(MOCK_TAG.workspaces.map(p => p._id))
      );

      sandbox.replace(
        TagModel,
        'validateTemplates',
        sandbox.stub().resolves(MOCK_TAG.templates.map(p => p._id))
      );

      sandbox.replace(
        TagModel,
        'validateProjects',
        sandbox.stub().resolves(MOCK_TAG.projects.map(p => p._id))
      );
      ...
      sandbox.replace(
        TagModel,
        'create',
        sandbox.stub().resolves([{_id: tagId}])
      );
  ...
  context('validate templates', () => {
    ...
    it('should return an array of ids when the templates can be validated', async () => {
      ...
      results.forEach(r => {
        const foundId = inputTemplates.find(
          p => p._id?.toString() === r.toString()
        );
        assert.isOk(foundId);
      });
  ...
  context('getTagById', () => {
    ...
    it('will retreive a project document with the projects populated', async () => {
      ...
      const doc = await TagModel.getTagById(
        mockTag._id as mongoose.Types.ObjectId
      );
    ...
    it('will throw a DataNotFoundError when the tag does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.getTagById(mockTag._id as mongoose.Types.ObjectId);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
  ...
  context('addProjects', () => {
    ...
    it('will add a project to a tag', async () => {
      ...
      const updatedTag = await TagModel.addProjects(projTypeId, [projectId]);

      assert.strictEqual(updatedTag._id, projTypeId);
      assert.strictEqual(
        updatedTag.projects[0].toString(),
        projectId.toString()
      );
    ...
    it('will not save when a project is already attached to an project type', async () => {
      ...
      const updatedTag = await TagModel.addProjects(tagId, [projectId]);
    ...
    it('will throw a data not found error when the project type does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.addProjects(tagId, [projectId]);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
    ...
    it('will throw a data validation error when project id does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.addProjects(projTypeId, [projectId]);
      } catch (err) {
        assert.instanceOf(err, error.DataValidationError);
        errored = true;
      }
    ...
    it('will throw an invalid argument error when the projects array is empty', async () => {
      ...
      const localMockTag = JSON.parse(JSON.stringify(MOCK_TAG));
      localMockTag._id = projectTypwId;
      localMockTag.projects = [];
      const projectId = new mongoose.Types.ObjectId();
      ...
      let errored = false;
      try {
        await TagModel.addProjects(projectTypwId, []);
      } catch (err) {
        assert.instanceOf(err, error.InvalidArgumentError);
        errored = true;
      }
  ...
  context('removeProjects', () => {
    ...
    it('will remove a project from the tag', async () => {
      ...
      const updatedTag = await TagModel.removeProjects(projTypeId, [projectId]);
    ...
    it('will not modify the projects if the projectid are not on the project type projects', async () => {
      ...
      const updatedTag = await TagModel.removeProjects(projTypeId, [
        new mongoose.Types.ObjectId(),
      ]);
    ...
    it('will throw a data not found error when the project type does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeProjects(projTypeId, [projectId]);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
    ...
    it('will throw a data operation error when the underlying connection fails', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeProjects(tagId, [projectId]);
      } catch (err) {
        assert.instanceOf(err, error.DatabaseOperationError);
        errored = true;
      }
    ...
    it('will throw an invalid argument error when the projects array is empty', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeProjects(tagId, []);
      } catch (err) {
        assert.instanceOf(err, error.InvalidArgumentError);
        errored = true;
      }
  ...
  context('addTemplates', () => {
    ...
    it('will add a template to a tag', async () => {
      ...
      const updatedTag = await TagModel.addTemplates(tagId, [templateId]);

      assert.strictEqual(updatedTag._id, tagId);
      assert.strictEqual(
        updatedTag.templates[0].toString(),
        templateId.toString()
      );
    ...
    it('will throw a data not found error when the tag does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.addTemplates(tagId, [templateId]);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
    ...
    it('will throw a data operation error when the underlying connection fails', async () => {
      ...
      let errored = false;
      try {
        await TagModel.addTemplates(tagId, [projectId]);
      } catch (err) {
        assert.instanceOf(err, error.DatabaseOperationError);
        errored = true;
      }
    ...
    it('will throw an invalid argument error when the templates array is empty', async () => {
      ...
      let errored = false;
      try {
        await TagModel.addTemplates(tagId, []);
      } catch (err) {
        assert.instanceOf(err, error.InvalidArgumentError);
        errored = true;
      }
  ...
  context('removeTemplates', () => {
    ...
    it('will remove a template from the tag', async () => {
      ...
      const updatedTag = await TagModel.removeTemplates(tagId, [templateId]);
    ...
    it('will not modify the template if the templateId is not on the tag templates', async () => {
      ...
      const updatedTag = await TagModel.removeTemplates(tagId, [
        new mongoose.Types.ObjectId(),
      ]);
    ...
    it('will throw a data not found error when the tag does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeTemplates(tagId, [templateId]);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
    ...
    it('will throw a data operation error when the underlying connection fails', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeTemplates(tagId, [projectId]);
      } catch (err) {
        assert.instanceOf(err, error.DatabaseOperationError);
        errored = true;
      }
    ...
    it('will throw an invalid argument error when the templates array is empty', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeTemplates(tagId, []);
      } catch (err) {
        assert.instanceOf(err, error.InvalidArgumentError);
        errored = true;
      }
  ...
  context('addWorkspaces', () => {
    ...
    it('will add a workspace to a tag', async () => {
      ...
      const updatedTag = await TagModel.addWorkspaces(tagId, [templateId]);

      assert.strictEqual(updatedTag._id, tagId);
      assert.strictEqual(
        updatedTag.workspaces[0].toString(),
        templateId.toString()
      );
    ...
    it('will not save when a workspace is already attached to an tag', async () => {
      ...
      const updatedTag = await TagModel.addWorkspaces(tagId, [workspaceId]);
    ...
    it('will throw a data not found error when the tag does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.addWorkspaces(tagId, [workspaceId]);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
    ...
    it('will throw an invalid argument error when the templates array is empty', async () => {
      ...
      let errored = false;
      try {
        await TagModel.addWorkspaces(tagId, []);
      } catch (err) {
        assert.instanceOf(err, error.InvalidArgumentError);
        errored = true;
      }
  ...
  context('removeWorkspaces', () => {
    ...
    it('will remove a workspace from the tag', async () => {
      ...
      const updatedTag = await TagModel.removeWorkspaces(tagId, [workspaceId]);
    ...
    it('will not modify the workspace if the workspaceId is not on the tag templates', async () => {
      ...
      const updatedTag = await TagModel.removeWorkspaces(tagId, [
        new mongoose.Types.ObjectId(),
      ]);
    ...
    it('will throw a data not found error when the tag does not exist', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeWorkspaces(tagId, [workspaceId]);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
    ...
    it('will throw a data operation error when the underlying connection fails', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeWorkspaces(tagId, [projectId]);
      } catch (err) {
        assert.instanceOf(err, error.DatabaseOperationError);
        errored = true;
      }
    ...
    it('will throw an invalid argument error when the templates array is empty', async () => {
      ...
      let errored = false;
      try {
        await TagModel.removeWorkspaces(tagId, []);
      } catch (err) {
        assert.instanceOf(err, error.InvalidArgumentError);
        errored = true;
      }
