...
describe('#mongoose/models/annotation', () => {
  ...
  context('allAnnotationIdsExist', () => {
    ...
    it('should return true when all the annotation ids exist', async () => {
      const annotationIds = [new mongoose.Types.ObjectId(), new mongoose.Types.ObjectId()];
    ...
    it('should throw a DataNotFoundError when one of the ids does not exist', async () => {
      ...
      try {
        ...
        assert.strictEqual(err.data.value[0].toString(), annotationIds[1].toString());
        errored = true;
      }
  ...
  context('validateUpdateObject', () => {
    ...
    it('will not throw an error when no unsafe fields are present', async () => {
      ...
      try {
        await AnnotationModel.validateUpdateObject(
          mocks.MOCK_ANNOTATION as unknown as Omit<Partial<databaseTypes.IAnnotation>, '_id'>
        );
      } catch (err) {
        errored = true;
      }
    ...
    it('will fail when trying to update the _id', async () => {
      ...
      try {
        ...
        } as unknown as Omit<Partial<databaseTypes.IAnnotation>, '_id'>);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
        errored = true;
      }
    ...
    it('will fail when trying to update the createdAt', async () => {
      ...
      try {
        await AnnotationModel.validateUpdateObject({...mocks.MOCK_ANNOTATION, createdAt: new Date()} as unknown as Omit<
          Partial<databaseTypes.IAnnotation>,
          '_id'
        >);
      } catch (err) {
        assert.instanceOf(err, error.InvalidOperationError);
    ...
    it('will fail when trying to update the updatedAt', async () => {
      ...
      try {
        await AnnotationModel.validateUpdateObject({...mocks.MOCK_ANNOTATION, updatedAt: new Date()} as unknown as Omit<
          Partial<databaseTypes.IAnnotation>,
          '_id'
        >);
      } catch (err) {
  ...
  context('createAnnotation', () => {
    ...
    it('will create a annotation document', async () => {
      sandbox.replace(AnnotationModel, 'validateAuthor', sandbox.stub().resolves(mocks.MOCK_ANNOTATION.author));
      sandbox.replace(AnnotationModel, 'validateState', sandbox.stub().resolves(mocks.MOCK_ANNOTATION.state));

      const objectId = new mongoose.Types.ObjectId();
      sandbox.replace(AnnotationModel, 'create', sandbox.stub().resolves([{_id: objectId}]));
    ...
    it('will rethrow a DataValidationError when the author validator throws one', async () => {
      sandbox.replace(
        AnnotationModel,
        'validateAuthor',
        sandbox.stub().rejects(new error.DataValidationError('The author does not exist', 'author ', {}))
      );
      sandbox.replace(AnnotationModel, 'validateState', sandbox.stub().resolves(mocks.MOCK_ANNOTATION.state));
    ...
    it('will rethrow a DataValidationError when the state validator throws one', async () => {
      ...
      sandbox.replace(
        AnnotationModel,
        'validateState',
        sandbox.stub().rejects(new error.DataValidationError('The state does not exist', 'state ', {}))
      );
    ...
    it('will throw a DatabaseOperationError when an underlying model function errors', async () => {
      ...
      const objectId = new mongoose.Types.ObjectId();
      sandbox.replace(AnnotationModel, 'validate', sandbox.stub().resolves(true));
      sandbox.replace(AnnotationModel, 'create', sandbox.stub().rejects('oops, something bad has happened'));
    ...
    it('will rethrow a DataValidationError when the validate method on the model errors', async () => {
      ...
      const objectId = new mongoose.Types.ObjectId();
      sandbox.replace(AnnotationModel, 'validate', sandbox.stub().rejects('oops an error has occurred'));
      sandbox.replace(AnnotationModel, 'create', sandbox.stub().resolves([{_id: objectId}]));
      const stub = sandbox.stub();
      stub.resolves({_id: objectId});
      sandbox.replace(AnnotationModel, 'getAnnotationById', stub);
  ...
  context('getAnnotationById', () => {
    ...
    it('will retreive a annotation document with the related fields populated', async () => {
      ...
      const doc = await AnnotationModel.getAnnotationById(mocks.MOCK_ANNOTATION._id as mongoose.Types.ObjectId);
    ...
    it('will throw a DataNotFoundError when the annotation does not exist', async () => {
      ...
      let errored = false;
      try {
        await AnnotationModel.getAnnotationById(mocks.MOCK_ANNOTATION._id as mongoose.Types.ObjectId);
      } catch (err) {
        assert.instanceOf(err, error.DataNotFoundError);
        errored = true;
      }
  ...
  context('queryAnnotations', () => {
    ...
    it('will return the filtered annotations', async () => {
      sandbox.replace(AnnotationModel, 'count', sandbox.stub().resolves(mockAnnotations.length));

      sandbox.replace(AnnotationModel, 'find', sandbox.stub().returns(new MockMongooseQuery(mockAnnotations)));
  ...
  context('updateAnnotationById', () => {
    ...
    it('Should update a annotation', async () => {
      ...
      const result = await AnnotationModel.updateAnnotationById(annotationId, updateAnnotation);
