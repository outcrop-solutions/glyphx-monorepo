   ...
   pub fn from_mongo_db_error(error: &ErrorKind, collection: &str, operation: &str) -> Self {
       ...
       match error {
           ErrorKind::BsonSerialization(err) => {
               ...
               DeleteDocumentError::DocumentSerializationError(GlyphxErrorData::new(
                   outer_message,
                   Some(data),
                    Some(inner_error),
                ))
           ...
           ErrorKind::Authentication { message, .. } => {
               ...
               DeleteDocumentError::AuthenticationError(GlyphxErrorData::new(
                   outer_message,
                   Some(data),
                    Some(inner_error),
                ))
           ...
           ErrorKind::InvalidArgument { message, .. } => {
               ...
               let inner_error = json!({"message" : message});
               DeleteDocumentError::InvalidQuery(GlyphxErrorData::new(
                   outer_message,
                   Some(data),
                    Some(inner_error),
                ))
           ...
           _ => {
               ...
               DeleteDocumentError::UnexpectedError(GlyphxErrorData::new(
                   outer_message,
                   Some(data),
                    Some(inner_error),
                ))
