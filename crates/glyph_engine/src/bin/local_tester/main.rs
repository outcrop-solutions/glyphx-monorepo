use glyph_engine::{errors::*, GlyphEngine, types::vectorizer_parameters::{FieldDefinition, VectorizerParameters}, vector_processer::{TaskStatus, VectorProcesser, VectorValueProcesser}, GlyphEngineOperations, GlyphEngineOperationsImpl};
...
impl GlyphEngineOperations for GlyphEngineOperationsStruct {
    async fn build_s3_connection(&self) -> Result<&'static S3Connection, GlyphEngineInitError> {
        GlyphEngineOperationsImpl.build_s3_connection().await
    }
    ...
    async fn start_athena_query(
        ...
        GlyphEngineOperationsImpl.start_athena_query(athena_connection, query).await
    }
    ...
    async fn check_query_status(
        ...
        GlyphEngineOperationsImpl.check_query_status(athena_connection, query_id).await
    }
    ...
    async fn get_query_results(
        ...
        GlyphEngineOperationsImpl.get_query_results(query_id, athena_connection).await
    }
    ...
    async fn get_upload_stream(
        ...
        GlyphEngineOperationsImpl.get_upload_stream(file_name, s3_connection).await
    }
    ...
    async fn write_to_upload_stream(
        ...
        GlyphEngineOperationsImpl.write_to_upload_stream(upload_stream, bytes).await
    }
    ...
    async fn finish_upload_stream(
        ...
        GlyphEngineOperationsImpl.finish_upload_stream(upload_stream).await
    }
    ...
    async fn add_process_tracking_error(
        ...
        GlyphEngineOperationsImpl.add_process_tracking_error(process_id, error).await
    }
    ...
    async fn complete_process_tracking(
        ...
        GlyphEngineOperationsImpl.complete_process_tracking(process_id, process_result, process_value).await
