...
mod from_from_json_error {
    ...
    fn is_ok() {
        ...
        let result = GetFieldDefinitionTypeError::from_from_json_error(input);
        match result {
...
mod from_get_field_definition_error {
    ...
    fn axis_not_defined() {
        ...
        let result = GetFieldDefinitionTypeError::from_get_field_definition_error(input);
        match result {
            GetFieldDefinitionTypeError::AxisNotDefined(error_data) => {
                assert_eq!(error_data.message, message);
                let d = error_data.data.unwrap();
                let field = d["field"].as_str().unwrap();
                assert_eq!(field, "test");
    ...
    fn supporting_field_not_defined() {
        ...
        let result = GetFieldDefinitionTypeError::from_get_field_definition_error(input);
        match result {
            GetFieldDefinitionTypeError::SupportingFieldNotDefined(error_data) => {
                assert_eq!(error_data.message, message);
                let d = error_data.data.unwrap();
                let field = d["field"].as_str().unwrap();
                assert_eq!(field, "test");
