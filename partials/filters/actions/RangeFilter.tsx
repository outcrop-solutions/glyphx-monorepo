...
export const RangeFilter = ({ setVisible, lastDroppedItem }) => {
  ...
  function deleteMaxMinFilter(){
    setFiltersApplied((prev)=>{
      return prev.filter((internalFilter)=>{ // remove this range filter here
  ...
  return (
    ...
    onKeyPress={(ev) => {
      ...
      {
        ishover ?
          <svg onClick={deleteMaxMinFilter} className=" h-5 w-5 rounded-full border-2 border-transparent hover:border-white hover:cursor-pointer bg-secondary-space-blue" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
